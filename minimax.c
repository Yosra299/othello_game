...
__declspec(dllexport) char getAvlbMoves(char* board, char color, char* moves)
{
    char i, j, k, n=0;
...
__declspec(dllexport) void updateBoard(char board[], char pos, char color, char * updated)
{
...
__declspec(dllexport) double evaluation(char* board, char color)
{
...
__declspec(dllexport) double endEval(char board[], char color)
{
	char b, w, e, i, sign = 0;
	sign = (color==nowplaying?1:-1);
	for(i=0; i<64; i++)
...
__declspec(dllexport) double alphaBeta(char board[], char depth, double alpha, double beta, char color)
{
...
__declspec(dllexport) int giveNextMove(char *pieces, char color)
{
